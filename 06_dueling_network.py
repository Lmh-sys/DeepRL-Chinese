...
class DeulingNetwork:
    def __init__(self, dim_state=None, num_action=None, discount=0.9):
        self.discount = discount
        self.V = VNet(dim_state)
        self.D = DNet(dim_state, num_action)
        
    def get_Q(self, state):
        dvals = self.D(state)
        vval = self.V(state)
        return dvals + vval - dvals.mean()
    ...
    def compute_loss(self, s_batch, a_batch, r_batch, d_batch, next_s_batch):
        ...
        dvals = self.D(s_batch).gather(1, a_batch.unsqueeze(1)).squeeze()
        # 计算Q value
        vval = self.V(s_batch).squeeze()
        qvals = dvals + vval - torch.mean(self.D(s_batch), 1)
        # 使用dueling network网络计算next_s_batch对应的值。
        next_qvals, _ = self.get_Q(next_s_batch).detach().max(dim=1)
        # 使用MSE计算loss。
...
def train(args, env, agent):
    ...
    optimizer = torch.optim.Adam(chain(agent.V.parameters(), agent.D.parameters()), lr=args.lr)
    optimizer.zero_grad()
    ...
    for i in range(args.max_steps):
        ...
        if done is True:
            ...
            if episode_reward > max_episode_reward:
                save_path = os.path.join(args.output_dir, "V_model.bin")
                torch.save(agent.V.state_dict(), save_path)
                save_path = os.path.join(args.output_dir, "D_model.bin")
                torch.save(agent.D.state_dict(), save_path)
...
def eval(args, env, agent):
    agent = DeulingNetwork(args.dim_state, args.num_action)
    v_model_path = os.path.join(args.output_dir, "V_model.bin")
    agent.V.load_state_dict(torch.load(v_model_path))

    d_model_path = os.path.join(args.output_dir, "D_model.bin")
    agent.D.load_state_dict(torch.load(d_model_path))
